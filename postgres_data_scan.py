...
# Load PostgreSQL credentials from file
...
def get_postgresql_connection(credentials):
    url = URL.create(
        ...
        port=credentials.get('port', 5432)  # Default PostgreSQL port
    )
    engine = create_engine(url, echo=False)  # echo=True to log SQL queries
    return engine


# Fetch data from PostgreSQL
def fetch_data_from_postgresql(engine, table_name, limit, offset):
...
def main(base_url, credentials_file, table_list_file, num_rows, output_file):
    data_source_name = "POSTGRESQL_DATA_SOURCE"
    credentials = load_credentials(credentials_file)
    auth_key = credentials["protecto_api_key"]
    table_list = load_table_list(table_list_file)
    engine = get_postgresql_connection(credentials)
    no_rows_per_call = 50
    report = {"details":[]}
    for table in table_list:
        print(f"Processing table: {table}")
        for i in range(0,num_rows,no_rows_per_call):
            columns, rows = fetch_data_from_postgresql(engine, table, no_rows_per_call, i)
            tracking_ids = []
